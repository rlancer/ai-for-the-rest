...
def create_from_project(
    ...
    print_only: bool = typer.Option(
        False,
        "--print",
        help="Print the template to the console without saving",
    ),
) -> None:
    ...
    for file_path, size in files:
        ...
        # Skip files generated from config or specific to template creation
        if rel_path_str in {"pyproject.toml", ".mise.toml", ".aftrignore"}:
            continue
    ...
    # Check if template already exists (skip when only printing)
    if not print_only and config.template_exists(template_name) and not force:
